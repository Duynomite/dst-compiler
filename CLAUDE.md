...
## Current State
[Updated 2026-03-27 — v3.0 Phase 3 Final: Verification Framework]
- **v3.0 Phase 3 DEPLOYED** at `duynomite.github.io/dst-compiler/`
- **Dual output pipeline**: `curated_disasters.json` (166 records, non-FEMA) + `all_disasters.json` (217 total, curated + FEMA DR/EM/FM)
- **166 curated records** (66 FMCSA + 9 SBA + 90 STATE + 1 HHS)
- **Carrier acknowledgment badges**: 93/166 records have carrier acks (Aetna 78, Wellcare 59, Humana 27). UI shows colored pills per carrier on DST cards.
- **Tiered verification framework**: 33 incidentEnd corrections with T1-T5 confidence tiers per 42 CFR 407.23(b)(2). Each has `endDateSource` + `endDateConfidence` metadata. 7 estimate-based corrections reverted to ongoing (insufficient evidence).
- **22 expired records removed** (fires contained, storms ended, declarations terminated — all verified via T1-T5 sources).
- **21 broken URLs fixed** (OR×6, CA×8, MO×3, NE×2, NY, IN, MT, CT, ME).
- **HHS WA PHE resolved**: incidentEnd=2026-03-24 (90-day statutory limit, no renewal found).
- **PR records removed** (not a CPC market).
- **Audit**: 3,821/3,821 + 4,994/4,994 (100%, zero failures)
...
| 2026-03-27 | v3.0 Phase 3 continued: Gap research + URL fixes + carrier badges | (1) Removed PR records (not a CPC market). (2) Re-ran carrier cross-reference: 44 gaps → 11 expired FM + 10 already in data + 8 added + 2 skipped + 13 dupes. (3) 8 new gap records: OR drought, AR tornadoes, AZ Gila flooding, CO western flooding, DC Potomac sewer, KS July storms, NC WNC wildfires, SC Covington fire. (4) 21 broken URLs fixed (OR×6, CA×8, MO×3, NE×2, NY, IN, MT, CT, ME). (5) HHS WA PHE resolved: incidentEnd=2026-03-24 (90-day statutory limit). (6) Carrier acknowledgment badges: inject_carrier_acknowledgments() post-processes with carrier_analysis.json + Humana crossref. 93/166 records matched (Aetna 78, Wellcare 59, Humana 27). CarrierBadges UI component with distinct colors per carrier (purple/blue/green/orange). (7) Deployed to separate dst-compiler repo for GitHub Pages. | v3.0 Phase 3+ ✅ | 4327+5500 (100%, 0 failures) |
| 2026-03-27 | v3.0 Phase 3 final: Ongoing records audit + verification framework | (1) Audited 79 STATE "ongoing" records: 40 had no evidence of continuation (fires contained, storms ended, floods receded). (2) Built tiered verification framework per 42 CFR 407.23(b)(2): T1=declaration text, T2=governor termination, T3=official containment (CAL FIRE/InciWeb), T4=statutory auto-expire (verified), T5=carrier+research aligned. (3) Applied 33 verified corrections: T1(3) + T2(3) + T3(15) + T4(5) + T5(7). Reverted 7 estimate-based corrections (NE×2, CA×3, NM, IN, MA) back to ongoing — insufficient evidence. (4) 22 expired records removed. Each correction has endDateSource + endDateConfidence metadata. (5) 166 curated + 51 FEMA = 217 total. | v3.0 Phase 3 Final ✅ | 3821+4994 (100%, 0 failures) |
